...
const STATUS_COLORS: Record<StatusDokumentu, "default" | "secondary" | "success" | "warning" | "destructive" | "outline"> = {
  szkic: "secondary",
...
export default function DokumentyPage() {
  ...
  const { data: dokumenty = [], isLoading } = useQuery<Dokument[]>({
    ...
    queryFn: () => dokumentyApi.list(filterTyp ? { typ: filterTyp, limit: 100 } : { limit: 100 }),
  });
  ...
  const approveMutation = useMutation({
    mutationFn: (id: string) =>
      dokumentyApi.update(id, { status: "zatwierdzony" as StatusDokumentu }),
  ...
  return (
    <div>
      <Header
        title="Dokumenty"
        description="Metryki, zaświadczenia i pisma parafialne"
      />
