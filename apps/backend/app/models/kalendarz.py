...
class Wydarzenie(Base, UUIDMixin, TimestampMixin, SoftDeleteMixin):
    ...
    grupa_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("grupy_parafialne.id", ondelete="SET NULL"),
        nullable=True,
        index=True,
    )
    ...
    wspolnota: Mapped["Wspolnota | None"] = relationship("Wspolnota", foreign_keys=[wspolnota_id])
    grupa: Mapped["GrupaParafialna | None"] = relationship(
        "GrupaParafialna", foreign_keys=[grupa_id]
    )
...
from app.models.grupy import GrupaParafialna  # noqa: E402, F401
